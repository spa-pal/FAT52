  ...
  46  000b               _rs485_cnt:
  47  000b 00            	dc.b	0
 106                     ; 115 void gran_char(signed char *adr, signed char min, signed char max)
 106                     ; 116 {
 108                     	switch	.text
 109  0000               _gran_char:
 111  0000 89            	pushw	x
 112       00000000      OFST:	set	0
 115                     ; 117 if (*adr<min) *adr=min;
 117  0001 9c            	rvf
 118  0002 f6            	ld	a,(x)
 ...
 143  0019 85            	popw	x
 144  001a 81            	ret
 197                     ; 122 void gran(signed short *adr, signed short min, signed short max)
 197                     ; 123 {
 198                     	switch	.text
 199  001b               _gran:
 201  001b 89            	pushw	x
 202       00000000      OFST:	set	0
 205                     ; 124 if (*adr<min) *adr=min;
 207  001c 9c            	rvf
 ...
 372  0089               L521:
 373                     ; 165 while (tx_counter1 == TX_BUFFER_SIZE1);
 375  0089 b631          	ld	a,_tx_counter1
 376  008b a132          	cp	a,#50
 377  008d 27fa          	jreq	L521
 378                     ; 167 if (tx_counter1 || ((UART1->SR & UART1_SR_TXE)==0))
 ...
 386  009a               L331:
 387                     ; 169    tx_buffer1[tx_wr_index1]=c;
 389  009a 5f            	clrw	x
 ...
 397                     ; 170    if (++tx_wr_index1 == TX_BUFFER_SIZE1) tx_wr_index1=0;
 ...
 411  00b2               L731:
 412                     ; 175 UART1->CR2|= UART1_CR2_TIEN | UART1_CR2_TCIEN;
 ...
 697  0158 81            	ret
 761                     ; 234 void ascii2hex(char *ptr_dst, char *ptr_src, short len)
 761                     ; 235 {
 762                     	switch	.text
 763  0159               _ascii2hex:
 765  0159 89            	pushw	x
 ...
 770                     ; 238 for(i=0;i<len/2;i++)
 772  015c 5f            	clrw	x
 773  015d 1f02          	ldw	(OFST-1,sp),x
 775  015f 2029          	jra	L303
 776  0161               L772:
 777                     ; 240 	ptr_dst[i]=((ascii2halFhex(ptr_src[i*2]))<<4)+((ascii2halFhex(ptr_src[(i*2)+1])));
 ...
 832  0008 00            	dc.b	0
 833  0009 00            	ds.b	1
 914                     ; 248 int str2int(char *ptr, char len)
 914                     ; 249 {
 915                     	switch	.text
 916  0199               _str2int:
 918  0199 89            	pushw	x
 ...
 923                     ; 253 @near char temp[10]={0,0,0,0,0,0,0,0,0};
 ...
 931                     ; 254 @near int temp_out=0;
 933  01a9 5f            	clrw	x
 934  01aa 1f03          	ldw	(OFST-13,sp),x
 935                     ; 258 for (i=0;i<len;i++)
...
1179                     ; 298 	ascii2hex(&bat_mod_dump[1][0],&rx_buffer[13+40],40);
...
1189                     ; 299 	ascii2hex(&bat_mod_dump[2][0],&rx_buffer[13+80],40);
...
1199                     ; 300 	ascii2hex(&bat_mod_dump[3][0],&rx_buffer[13+120],40);
...
1209                     ; 301 	ascii2hex(&bat_mod_dump[4][0],&rx_buffer[13+160],40);
...
1219                     ; 302 	ascii2hex(&bat_mod_dump[5][0],&rx_buffer[13+200],40);
...
1229                     ; 303 	ascii2hex(&bat_mod_dump[6][0],&rx_buffer[13+240],40);
...
1260                     ; 314 	ascii2hex(&bat_mod_dump[1][20],&rx_buffer[21+34],34);
...
1270                     ; 315 	ascii2hex(&bat_mod_dump[2][20],&rx_buffer[21+68],34);
...
1280                     ; 316 	ascii2hex(&bat_mod_dump[3][20],&rx_buffer[21+102],34);
...
1290                     ; 317 	ascii2hex(&bat_mod_dump[4][20],&rx_buffer[21+136],34);
...
1300                     ; 318 	ascii2hex(&bat_mod_dump[5][20],&rx_buffer[21+170],34);
...
1310                     ; 319 	ascii2hex(&bat_mod_dump[6][20],&rx_buffer[21+204],34);
...
1327                     ; 327 }
1330  0390 81            	ret
1353                     ; 330 void init_CAN(void) {
1354                     	switch	.text
1355  0391               _init_CAN:
1359                     ; 331 	CAN->MCR&=~CAN_MCR_SLEEP;					// CAN wake up request
1361  0391 72135420      	bres	21536,#1
1362                     ; 332 	CAN->MCR|= CAN_MCR_INRQ;					// CAN initialization request
1364  0395 72105420      	bset	21536,#0
1366  0399               L344:
1367                     ; 333 	while((CAN->MSR & CAN_MSR_INAK) == 0);	// waiting for CAN enter the init mode
1369  0399 c65421        	ld	a,21537
1370  039c a501          	bcp	a,#1
1371  039e 27f9          	jreq	L344
1372                     ; 335 	CAN->MCR|= CAN_MCR_NART;					// no automatic retransmition
1374  03a0 72185420      	bset	21536,#4
1375                     ; 337 	CAN->PSR= 2;							// *** FILTER 0 SETTINGS ***
1377  03a4 35025427      	mov	21543,#2
1378                     ; 346 	CAN->Page.Filter01.F0R1= MY_MESS_STID>>3;			// 16 bits mode
1380  03a8 35135428      	mov	21544,#19
1381                     ; 347 	CAN->Page.Filter01.F0R2= MY_MESS_STID<<5;
1383  03ac 35c05429      	mov	21545,#192
1384                     ; 348 	CAN->Page.Filter01.F0R5= MY_MESS_STID_MASK>>3;
1386  03b0 357f542c      	mov	21548,#127
1387                     ; 349 	CAN->Page.Filter01.F0R6= MY_MESS_STID_MASK<<5;
1389  03b4 35e0542d      	mov	21549,#224
1390                     ; 352 	CAN->PSR= 6;									// set page 6
1392  03b8 35065427      	mov	21543,#6
1393                     ; 357 	CAN->Page.Config.FMR1&=~3;								//mask mode
1395  03bc c65430        	ld	a,21552
1396  03bf a4fc          	and	a,#252
1397  03c1 c75430        	ld	21552,a
1398                     ; 363 	CAN->Page.Config.FCR1= ((3<<1) & (CAN_FCR1_FSC00 | CAN_FCR1_FSC01));		//16 bit scale
1400  03c4 35065432      	mov	21554,#6
1401                     ; 366 	CAN->Page.Config.FCR1|= CAN_FCR1_FACT0;	// filter 0 active
1403  03c8 72105432      	bset	21554,#0
1404                     ; 369 	CAN->PSR= 6;								// *** BIT TIMING SETTINGS ***
1406  03cc 35065427      	mov	21543,#6
1407                     ; 371 	CAN->Page.Config.BTR1= 9;					// CAN_BTR1_BRP=9, 	tq= fcpu/(9+1)
1409  03d0 3509542c      	mov	21548,#9
1410                     ; 372 	CAN->Page.Config.BTR2= (1<<7)|(6<<4) | 7; 		// BS2=8, BS1=7, 		
1412  03d4 35e7542d      	mov	21549,#231
1413                     ; 374 	CAN->IER|=(1<<1);
1415  03d8 72125425      	bset	21541,#1
1416                     ; 377 	CAN->MCR&=~CAN_MCR_INRQ;					// leave initialization request
1418  03dc 72115420      	bres	21536,#0
1420  03e0               L154:
1421                     ; 378 	while((CAN->MSR & CAN_MSR_INAK) != 0);	// waiting for CAN leave the init mode
1423  03e0 c65421        	ld	a,21537
1424  03e3 a501          	bcp	a,#1
1425  03e5 26f9          	jrne	L154
1426                     ; 379 }
1429  03e7 81            	ret
1537                     ; 382 void can_transmit(unsigned short id_st,char data0,char data1,char data2,char data3,char data4,char data5,char data6,char data7)
1537                     ; 383 {
1538                     	switch	.text
1539  03e8               _can_transmit:
1541  03e8 89            	pushw	x
1542       00000000      OFST:	set	0
1545                     ; 385 if((can_buff_wr_ptr<0)||(can_buff_wr_ptr>9))can_buff_wr_ptr=0;
1547  03e9 b627          	ld	a,_can_buff_wr_ptr
1548  03eb a10a          	cp	a,#10
1549  03ed 2502          	jrult	L335
1552  03ef 3f27          	clr	_can_buff_wr_ptr
1553  03f1               L335:
1554                     ; 387 can_out_buff[can_buff_wr_ptr][0]=(char)(id_st>>6);
...
1568                     ; 388 can_out_buff[can_buff_wr_ptr][1]=(char)(id_st<<2);
...
1578                     ; 390 can_out_buff[can_buff_wr_ptr][2]=data0;
...
1586                     ; 391 can_out_buff[can_buff_wr_ptr][3]=data1;
...
1594                     ; 392 can_out_buff[can_buff_wr_ptr][4]=data2;
...
1602                     ; 393 can_out_buff[can_buff_wr_ptr][5]=data3;
...
1610                     ; 394 can_out_buff[can_buff_wr_ptr][6]=data4;
...
1618                     ; 395 can_out_buff[can_buff_wr_ptr][7]=data5;
...
1626                     ; 396 can_out_buff[can_buff_wr_ptr][8]=data6;
...
1634                     ; 397 can_out_buff[can_buff_wr_ptr][9]=data7;
...
1642                     ; 399 can_buff_wr_ptr++;
1644  046a 3c27          	inc	_can_buff_wr_ptr
1645                     ; 400 if(can_buff_wr_ptr>9)can_buff_wr_ptr=0;
...
1657  0474 85            	popw	x
1658  0475 81            	ret
1687                     ; 404 void can_tx_hndl(void)
1687                     ; 405 {
1688                     	switch	.text
1689  0476               _can_tx_hndl:
1693                     ; 406 if(bTX_FREE)
1695  0476 3d07          	tnz	_bTX_FREE
1696  0478 2754          	jreq	L745
1697                     ; 408 	if(can_buff_rd_ptr!=can_buff_wr_ptr)
...
1708                     ; 413 		CAN->Page.TxMailbox.MDLCR=8;
1710  0486 35085429      	mov	21545,#8
1711                     ; 414 		CAN->Page.TxMailbox.MIDR1=can_out_buff[can_buff_rd_ptr][0];
...
1719                     ; 415 		CAN->Page.TxMailbox.MIDR2=can_out_buff[can_buff_rd_ptr][1];
...
1727                     ; 417 		memcpy(&CAN->Page.TxMailbox.MDAR1, &can_out_buff[can_buff_rd_ptr][2],8);
...
1889                     ; 469 	bat_mod_dump[0][38]=((ascii2halFhex(rx_buffer[15]))<<4)+((ascii2halFhex(rx_buffer[16])));
...
1904                     ; 470 	bat_mod_dump[0][39]=((ascii2halFhex(rx_buffer[17]))<<4)+((ascii2halFhex(rx_buffer[18])));
...
1919                     ; 471 	bat_mod_dump[0][40]=((ascii2halFhex(rx_buffer[19]))<<4)+((ascii2halFhex(rx_buffer[20])));
...
1934                     ; 472 	bat_mod_dump[0][41]=rs485_cnt;
1936  0583 55000b0029    	mov	_bat_mod_dump+41,_rs485_cnt
1937                     ; 483 	can_transmit(0x18e,bat_mod_dump[transmit_cnt_number][0],PUT_LB_TM1+transmit_cnt_number,
1937                     ; 484 	bat_mod_dump[transmit_cnt_number][1],
1937                     ; 485 	bat_mod_dump[transmit_cnt_number][2],bat_mod_dump[transmit_cnt_number][3],
1937                     ; 486 	bat_mod_dump[transmit_cnt_number][4],bat_mod_dump[transmit_cnt_number][5],
1937                     ; 487 	bat_mod_dump[transmit_cnt_number][6]);
...
1988                     ; 488 	can_transmit(0x18e,bat_mod_dump[transmit_cnt_number][7],PUT_LB_TM2+transmit_cnt_number,
1988                     ; 489 	bat_mod_dump[transmit_cnt_number][8],
1988                     ; 490 	bat_mod_dump[transmit_cnt_number][9],bat_mod_dump[transmit_cnt_number][10],
1988                     ; 491 	bat_mod_dump[transmit_cnt_number][11],bat_mod_dump[transmit_cnt_number][12],
1988                     ; 492 	bat_mod_dump[transmit_cnt_number][13]);
...
2039                     ; 493 	can_transmit(0x18e,bat_mod_dump[transmit_cnt_number][14],PUT_LB_TM3+transmit_cnt_number,
2039                     ; 494 	bat_mod_dump[transmit_cnt_number][15],
2039                     ; 495 	bat_mod_dump[transmit_cnt_number][16],bat_mod_dump[transmit_cnt_number][17],
2039                     ; 496 	bat_mod_dump[transmit_cnt_number][18],bat_mod_dump[transmit_cnt_number][19],
2039                     ; 497 	bat_mod_dump[transmit_cnt_number][20]);
...
2090                     ; 498 	can_transmit(0x18e,bat_mod_dump[transmit_cnt_number][21],PUT_LB_TM4+transmit_cnt_number,
2090                     ; 499 	bat_mod_dump[transmit_cnt_number][22],
2090                     ; 500 	bat_mod_dump[transmit_cnt_number][23],bat_mod_dump[transmit_cnt_number][24],
2090                     ; 501 	bat_mod_dump[transmit_cnt_number][25],bat_mod_dump[transmit_cnt_number][26],
2090                     ; 502 	bat_mod_dump[transmit_cnt_number][27]);
...
2141                     ; 503 	can_transmit(0x18e,bat_mod_dump[transmit_cnt_number][28],PUT_LB_TM5+transmit_cnt_number,
2141                     ; 504 	bat_mod_dump[transmit_cnt_number][29],
2141                     ; 505 	bat_mod_dump[transmit_cnt_number][30],bat_mod_dump[transmit_cnt_number][31],
2141                     ; 506 	bat_mod_dump[transmit_cnt_number][32],bat_mod_dump[transmit_cnt_number][33],
2141                     ; 507 	bat_mod_dump[transmit_cnt_number][34]);
...
2192                     ; 508 	can_transmit(0x18e,bat_mod_dump[transmit_cnt_number][35],PUT_LB_TM6+transmit_cnt_number,
2192                     ; 509 	bat_mod_dump[transmit_cnt_number][36],
2192                     ; 510 	bat_mod_dump[transmit_cnt_number][37],bat_mod_dump[transmit_cnt_number][38],
2192                     ; 511 	bat_mod_dump[transmit_cnt_number][39],bat_mod_dump[transmit_cnt_number][40],
2192                     ; 512 	bat_mod_dump[transmit_cnt_number][41]);
...
2249                     ; 518 	transmit_cnt_number++;
2251  0780 3c00          	inc	_transmit_cnt_number
2252                     ; 519 	if(transmit_cnt_number>=7)transmit_cnt_number=0;
...
2260  078a               L165:
2261                     ; 529 can_in_an_end:
2261                     ; 530 bCAN_RX=0;
2263  078a 3f08          	clr	_bCAN_RX
2264                     ; 531 }   
2267  078c 84            	pop	a
2268  078d 81            	ret
2291                     ; 535 void t4_init(void){
2292                     	switch	.text
2293  078e               _t4_init:
2297                     ; 536 	TIM4->PSCR = 7;
2299  078e 35075345      	mov	21317,#7
2300                     ; 537 	TIM4->ARR= 77;
2302  0792 354d5346      	mov	21318,#77
2303                     ; 538 	TIM4->IER|= TIM4_IER_UIE;					// enable break interrupt
2305  0796 72105341      	bset	21313,#0
2306                     ; 540 	TIM4->CR1=(TIM4_CR1_URS | TIM4_CR1_CEN | TIM4_CR1_ARPE);	
2308  079a 35855340      	mov	21312,#133
2309                     ; 542 }
2312  079e 81            	ret
2347                     ; 548 @far @interrupt void TIM4_UPD_Interrupt (void) 
2347                     ; 549 {
2349                     	switch	.text
2350  079f               f_TIM4_UPD_Interrupt:
2354                     ; 550 TIM4->SR1&=~TIM4_SR1_UIF;
2356  079f 72115342      	bres	21314,#0
2357                     ; 553 if(++t0_cnt0>=10)
...
2436                     ; 605 }
2439  07fb 80            	iret
2464                     ; 608 @far @interrupt void CAN_RX_Interrupt (void) 
2464                     ; 609 {
2465                     	switch	.text
2466  07fc               f_CAN_RX_Interrupt:
2470                     ; 619 CAN->PSR= 7;
2472  07fc 35075427      	mov	21543,#7
2473                     ; 628 memcpy(&mess[0], &CAN->Page.RxFIFO.MFMI, 14); // compare the message content
...
2606                     ; 669 		if (++tx_rd_index1 == TX_BUFFER_SIZE1) tx_rd_index1=0;
...
2742  089c               L147:
2743                     ; 716 	if((rx_data==0x0d)&&(rs485_rx_cnt==264))bRX485=2;
...
2755  08ad               L347:
2756                     ; 718 	if(rx_data==0x0d)rs485_rx_cnt=0;	
...
3123  0a35               L621:
3124                     ; 880 			rs485_cnt=100;
3126  0a35 3564000b      	mov	_rs485_cnt,#100
3127                     ; 881 			bRS485ERR=1;
